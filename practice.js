...
function divider(numbersArray){
  ...
  dividerArr.push(evensArr);
  dividerArr.push(oddsArr);
...
function removeItem(myGroceryList, removedItem){
  //if item is in list, remove and return updated list
...
function addItem(myGroceryList, addedItem){
  // add item to myGrocerList and return new array
  for(let i=0; i< myGroceryList.length; i++){
    if(myGroceryList[i] !== addedItem){
...
function addTen(numbers){
  ...
  for(let i=0; i<numbers.length; i++){
    numbers.parseInt(numbers[i], 10);
    tenArr.push(numbers[i] + 10);
  }
  return tenArr;
}
...
function both(arr1, arr2){
  ...
  return //new array with matching numbers in both arrays. 
}
...
var user2 = {
  name: 'Daniel',
  email: 'ddominguez20.dd@gmai.com',
  password: 'password',
  username: 'functionFanatic'
};
var user3 = {
  name: 'Graphix',
  email: 'gddominguez18@aol.com',
  password: 'iLoveChess',
